...
from pysac.mhs_atmosphere.parameters.model_pars import paper1,paper2a,paper2b,\
                                         paper2c,paper2d,mfe_setup as test_pars
#==============================================================================
...
for model_pars in test_pars:
    ...
    if option_pars['l_mpi']:
        ...
        if rank == 0:
            out_vars = []
            for i in range(0,len(unit_vars)):
                out_vars.append(u.Quantity(np.concatenate(concat_vars[i], axis=0),
                unit=u.Unit(unit_vars[i])))
            rho,Bx,By,Bz,energy = out_vars
            del out_vars
        del gather_vars, concat_vars, unit_vars
        gather_vars = [
                       Fx.si.value,Fy.si.value
                      ]
        unit_vars = [Fx.unit.to_string(), Fy.unit.to_string()]
        concat_vars = []
        for var in gather_vars:
            concat_vars.append(comm.gather(var, root=0))
        ...
        unit_vars = [
                       pressure_m.unit.to_string(),
                       rho_m.unit.to_string(),
                       temperature.unit.to_string(),
                       pbeta.unit.to_string(),
                       alfven.unit.to_string(),
                       cspeed.unit.to_string(),
                       Btensx.unit.to_string(),
                       Btensy.unit.to_string()
                      ]
        concat_vars = []
        for var in gather_vars:
            concat_vars.append(comm.gather(var, root=0))
        ...
        if rank == 0:
            ...
            pressure_m, rho_m, temperature, pbeta,\
                alfven, cspeed, Btensx, Btensy = out_vars
            del out_vars
        del gather_vars, concat_vars
