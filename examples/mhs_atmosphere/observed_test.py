# -*- coding: utf-8 -*-
"""
Created on Monday Feb 18 2019

@author: BS

Load a fits file, find magnetic sources and create 3D atmosphere.

TO DO
1) Fix length scale and magnetic field for observation
2) Check boundary
3) Tweak parameters to create stable atmosphere
4) Check SAC stability
...
from pysac.mhs_atmosphere.parameters.model_pars import obs_test as model_pars #mfe_setup

import pysac.mhs_atmosphere.obs_test
...
physical_constants['gravity'] = -274.0*u.m/u.s**2
...
#==============================================================================
# load magnetogram and find flux tubes
#==============================================================================

#load the fits file
obv_bz=atm.obs_test.read_fits_bz('Bz_Heliocentric.fits')

#find flux tubes
xi,yi,Si=atm.obs_test.flat_fits(obv_bz,2)

xi *= 0.0769230769*u.Mm #0.1
xi -= 5.0*u.Mm
yi *= 0.0769230769*u.Mm #0.1
yi -= 5.0*u.Mm
Si *= 0.00001*u.T #0.0001 from G to T
...
print 'magnetic pressure has nans: ', np.isnan(pressure_m).any()
print 'magnetic density has nans: ', np.isnan(rho_m).any()
...
magp_meanz *= np.abs(np.mean(pressure_m[:,:,-1]))#**2/(2*physical_constants['mu0'])

magrho_meanz = np.ones(len(coords['Z'])) * u.one
magrho_meanz *= np.abs(np.mean(rho_m[:,:,0]))

#print magp_meanz

pressure_Z, rho_Z, Rgas_Z = atm.hs_atmosphere.vertical_profile_test(
                                                 coords['Z'],
                                                 table,
                                                 magp_meanz,
                                                 physical_constants,
...
#datadir = os.path.expanduser('/fastdata-sharc/sm1bjs2/mhs_atmosphere/'+model_pars['model']+'/')
